...
class Query():
    ...
    def alias(self, query: str, alias: str):
        self.command.update({'width': f'WITH ({query}) as {alias} '})
        return self
...
def add_expr(expr, cond = ''):
    return f'{cond} {expr} '

def add_join(right, left, on):
    return f'JOIN {right} ON {left}.{on} = {right}.{on} '
...
class Query():
    ...
    def with_clause(self, query: str, alias: str):
        self.command.update({'width': f'WITH ({query}) as {alias} '})
        return self
